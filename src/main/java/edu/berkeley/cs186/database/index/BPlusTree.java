...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        //Put the  new data on the root.
        Optional<Pair<DataBox, Long>> temp_node = root.put(key, rid);

        //if the root does not overflow, we are good. Just return
        if (!temp_node.isPresent()) {
            return;
        }

        //If the root does overflow, create a new root, then update the new root.
        ...
        //Get the keys
        newKeys.add(new_node.getFirst());

        //Get the children
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(new_node.getSecond());

        //Make the new root and update the root
        InnerNode new_root = new InnerNode(metadata, bufferManager, newKeys, newChildren, lockContext);
        updateRoot(new_root);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        //See if the iterator has some elements.
        //See if the iterator has some elements.
        //Put the  new data on the root.
        while (data.hasNext()) {

            Optional<Pair<DataBox, Long>> temp_node = root.bulkLoad(data, fillFactor);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            ...
            Optional<LeafNode> rightNode = currentNode.getRightSibling();
            while (rightNode.isPresent()) {
                currentNode = rightNode.get();
                iter = currentNode.scanAll();
                if (iter.hasNext()) {
                    return true;
                }
                rightNode = currentNode.getRightSibling();
            }
            return false;
        }
