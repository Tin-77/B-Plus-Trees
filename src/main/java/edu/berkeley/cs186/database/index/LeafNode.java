...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // if overflow, returns a pair of split_key and right_node_page_num
        List<DataBox> rightKeys = keys.subList(d, 2*d + 1);
        List<RecordId> rightRids = rids.subList(d, 2*d + 1);
        this.keys = keys.subList(0, d);
        this.rids = rids.subList(0, d);


        // create a new leaf node to store the new data
        LeafNode newLeafNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, this.rightSibling, treeContext);
        long siblingPageNum = newLeafNode.getPage().getPageNum();
        this.rightSibling = Optional.of(siblingPageNum);;
        sync();
        return Optional.of(new Pair(rightKeys.get(0), siblingPageNum));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int numKeys = (int) (2 * d * fillFactor);

        while(data.hasNext() && keys.size() < numKeys) {
            Pair<DataBox, RecordId> elem = data.next();
            DataBox key = elem.getFirst();
            RecordId rid = elem.getSecond();
            keys.add(key);
            rids.add(rid);
        }

        // no overflow syncs to disk and returns Optional.empty()
        if (keys.size() <= numKeys && !data.hasNext()) {
            sync();
            return Optional.empty();
        }
        ...
        // if overflow, returns a pair of split_key and right_node_page_num
        Pair<DataBox, RecordId> elem = data.next();
        List<DataBox> rightKeys = new ArrayList<DataBox>();
        rightKeys.add(elem.getFirst());
        List<RecordId> rightRids = new ArrayList<>();
        rightRids.add(elem.getSecond());
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        //Check if the nodetype is a leafnode
        assert(nodeType == (byte) 1);

        //Get PageId of right sibling
        Long rightSibling = buf.getLong();
        Optional<Long> rightSiblingNode;
        if (rightSibling == -1) {
            rightSiblingNode = Optional.empty();
        } else {
            rightSiblingNode = Optional.of(rightSibling);
        }

        //Get number of (key, rid) pairs
        int keyIDsize = buf.getInt();
        //populate (key, id) pairs
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, rightSiblingNode, treeContext);
    }
