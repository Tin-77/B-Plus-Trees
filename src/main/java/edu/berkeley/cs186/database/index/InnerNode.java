...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        // get the child Node
        BPlusNode childNode = getChild(idx);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // find the right position for the child key
        int idx = numLessThanEqual(key, keys);
        BPlusNode childNode = getChild(idx);

        Optional<Pair<DataBox, Long>> temp_node = childNode.put(key, rid);

        // check if we need to store key in InnerNode
        if (temp_node.isPresent()) {

            Pair<DataBox, Long> new_node = temp_node.get();

            // get the key that need to put in the InnerNode
            DataBox childKey  = new_node.getFirst();

            // get the child's page number that need to put in the InnerNode
            long childPageNum  = new_node.getSecond();

            int d = metadata.getOrder();
            keys.add(idx, childKey);
            children.add(idx+1, childPageNum);

            // no overflow syncs to disk and returns Optional.empty()
            if (keys.size() <= d * 2) {
                // add key and childPageNum into the existing ArrayLists
                sync();
            ...
            // create a new InnerNode to store the new data
            InnerNode newInnerNode = new InnerNode(metadata, bufferManager, rightKeys, rightChildren, treeContext);
            long innerPageNum = newInnerNode.getPage().getPageNum();
            sync();
            return Optional.of(new Pair(pushKey, innerPageNum));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        //See if the iterator has some elements and fill the node until it is full
        while (data.hasNext() && keys.size() <= 2*d) {

            BPlusNode rightChildNode = getChild(children.size() - 1);

            Optional<Pair<DataBox, Long>> temp_node = rightChildNode.bulkLoad(data, fillFactor);
            ...
            if (temp_node.isPresent()) {
                ...
                keys.add(keys.size(), childKey);
                children.add(keys.size(), childPageNum);

            }
        }

        // no overflow syncs to disk and returns Optional.empty()
        if (keys.size() <= d * 2 && !data.hasNext()) {
            // add key and childPageNum into the existing ArrayLists
            sync();
            return Optional.empty();
        }

        //if the node is full and there are still some elements left, we split
        DataBox pushKey = keys.get(d);
        List<DataBox> rightKeys = keys.subList(d+1, 2*d+1);
        List<Long> rightChildren = children.subList(d+1, 2*d+2);
        this.keys = keys.subList(0, d);
