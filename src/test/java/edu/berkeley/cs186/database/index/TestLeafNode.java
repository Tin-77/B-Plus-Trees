...
public class TestLeafNode {
    ...
    public void testGetL_withKeys() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            assertEquals(leaf, leaf.get(keys.get(i)));
        }
    }

    @Test
    @Category(PublicTests.class)
    public void testGet2L() {
        setBPlusTreeMetadata(Type.intType(), 5);
        LeafNode leaf1 = getEmptyLeaf(Optional.empty());
        LeafNode leaf2 = getEmptyLeaf(Optional.empty());
        for (int i = 0; i < 10; ++i) {
            assertEquals(leaf1, leaf1.get(new IntDataBox(i)));
            assertEquals(leaf2, leaf2.get(new IntDataBox(i)));
            assertNotEquals(leaf1, leaf2.get(new IntDataBox(i)));
            assertNotEquals(leaf2, leaf1.get(new IntDataBox(i)));
        }
    }
    ...
    @Test
    @Category(PublicTests.class)
    public void testWithOverflowPutsFromDisk_d5() {
        // test overflows for put with d = 5.
        int d = 5;
        setBPlusTreeMetadata(Type.intType(), d);
        LeafNode leaf = getEmptyLeaf(Optional.empty());
        ...
        //Get the leaf's right sibling
        Optional<LeafNode> rightSiblingLeaf = leaf.getRightSibling();

        long rightSibPageNum = rightSiblingLeaf.get().getPage().getPageNum();

        //Read the right sibling leaf from disk
        LeafNode rightSibFromDisk = LeafNode.fromBytes(metadata, bufferManager, treeContext, rightSibPageNum);

        for (int i = d; i <= 2*d ; ++i) {
            IntDataBox key = new IntDataBox(i);
            RecordId rid = new RecordId(i, (short) i);
            assertEquals(Optional.of(rid), rightSibFromDisk.getKey(key));
        }
    }

    @Test
    @Category(PublicTests.class)
    public void testWithOverflowPutsFromDisk_d3() {
        // test overflow for put with d = 3.
        int d = 3;
        setBPlusTreeMetadata(Type.intType(), d);
        LeafNode leaf = getEmptyLeaf(Optional.empty());
    ...
    @Test
    @Category(PublicTests.class)
    public void testAbsentAndExistingRemoves() {
        // Puts 2d keys into the leaf, then removes an absent entry.
        // Requires put, get and remove to be implemented to pass.
        ...
        IntDataBox key = new IntDataBox(11);
        leaf.remove(key);
        ...
        for (int i = 1; i < 2 * d; ++i) {
            IntDataBox k = new IntDataBox(i);
            RecordId rid = new RecordId(i, (short) i);
            assertEquals(Optional.of(rid), leaf.getKey(k));
        }
    }

    @Test
    @Category(PublicTests.class)
    public void testRemovesWithOverflow() {
        // Puts 2d keys into the leaf, then removes and checks that they can no
        // be retrieved.
        // Requires put, get and remove to be implemented to pass.
        int d = 3;
        ...
        //Insert data that will create an overflow
        leaf.put(new IntDataBox(6), new RecordId(6, (short) 6));

        // Remove entries from the original leaf.
        for (int i = 0; i < d; ++i) {
            IntDataBox key = new IntDataBox(i);
            leaf.remove(key);
            assertEquals(Optional.empty(), leaf.getKey(key));
        ...
        //See if the overflow made a new leaf node and then remove, check if its not there anymore
        for (int i = d; i <= 2*d; ++i) {
            IntDataBox key = new IntDataBox(i);
            RecordId rid = new RecordId(i, (short) i);
            assertEquals(Optional.of(rid), rightSibFromDisk.getKey(key));
            rightSibFromDisk.remove(key);
            assertEquals(Optional.empty(), rightSibFromDisk.getKey(key));
        }
    }
    ...
    @Test
    @Category(PublicTests.class)
    public void testToAndFromBytes_NoRightSibling() {
        // Tests that leaf nodes without Right sibling are properly serialized and deserialized
